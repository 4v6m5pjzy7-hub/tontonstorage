...
import { money, dollarsInWords, prettyDate, TERM_LABELS, dueAtSigning, initialsOf } from './format.js';

// Once signed electronically, the initials blanks are filled in from the
// signer's own typed legal name rather than left empty.
function initialMark(text) {
  return `<span style="border-bottom:1px solid #111;padding:0 12px 1px;font-weight:700;letter-spacing:.5px;">${text}</span>`;
}
function initialsLine(t, { provider = false } = {}) {
  const tenant = t.signature?.name ? initialsOf(t.signature.name) : '';
  const prov = t.providerSignature?.name ? initialsOf(t.providerSignature.name) : '';
  const tenantPart = `Tenant Initials: ${tenant ? initialMark(tenant) : '________'}`;
  if (!provider) return tenantPart;
  return `${tenantPart} &nbsp;&nbsp;&nbsp; Provider Initials: ${prov ? initialMark(prov) : '________'}`;
}
...
export function renderContract(r) {
  ...
  Payment: ${box(t.paymentSchedule === 'monthly')} Monthly &nbsp; ${box(t.paymentSchedule === 'prepaid')} Prepaid &nbsp; Agreement start: <span class="fill">${prettyDate(t.startDate)}</span><br>
  ${t.termStart && t.endDate
    ? `Term period: <span class="fill">${prettyDate(t.termStart)} through ${prettyDate(t.endDate)}</span><br>`
    : t.endDate ? `Term period: <span class="fill">${prettyDate(t.startDate)} through ${prettyDate(t.endDate)}</span><br>` : ''}
  ...
  ${t.proration ? `<p><strong>Prorated First Month:</strong> This Agreement is entered mid-month. Storage from ${prettyDate(t.proration.from)} through ${prettyDate(t.proration.to)} (${t.proration.days} of ${t.proration.daysInMonth} days) is prorated at ${money(t.proration.amount)}. The fixed term then begins ${prettyDate(t.termStart)}.</p>` : ''}

  <p><strong>Advance Payment (Due at Signing):</strong> Tenant shall pay the following upon execution of this Agreement:</p>
  <table style="width:100%;border-collapse:collapse;margin:6px 0 10px;">
    ${t.proration ? `<tr><td style="padding:4px 0;">Prorated storage (${prettyDate(t.proration.from)} - ${prettyDate(t.proration.to)})</td><td style="padding:4px 0;text-align:right;">${money(due.prorated)}</td></tr>` : ''}
    ${due.prepaid
      ? `<tr><td style="padding:4px 0;">Full term prepaid (${due.months} months at ${money(t.monthlyFee)})</td><td style="padding:4px 0;text-align:right;">${money(due.termTotal)}</td></tr>`
      : `<tr><td style="padding:4px 0;">First month</td><td style="padding:4px 0;text-align:right;">${money(due.firstMonth)}</td></tr>
         <tr><td style="padding:4px 0;">Last month (held as prepaid rent)</td><td style="padding:4px 0;text-align:right;">${money(due.lastMonth)}</td></tr>`}
    ${due.oneTime ? `<tr><td style="padding:4px 0;">${esc(due.oneTimeLabel)}</td><td style="padding:4px 0;text-align:right;">${money(due.oneTime)}</td></tr>` : ''}
    <tr><td style="padding:6px 0;border-top:1px solid #333;font-weight:800;">TOTAL DUE AT SIGNING</td><td style="padding:6px 0;border-top:1px solid #333;text-align:right;font-weight:800;">${money(due.total)} (${dollarsInWords(due.total)})</td></tr>
  </table>
  ${due.prepaid
    ? `<p>Tenant has elected to prepay the entire storage term in advance. No monthly invoicing will follow for the prepaid term.</p>`
    : `<p>The last month's payment shall be held by Provider as prepaid rent and applied to the final month of the storage term. In the event the term is extended by any addendum or written agreement, the prepaid last month's rent shall automatically transfer to and be applied against the final month of the extension period, and shall not be applied at the end of the original term.</p>`}
  ...
  <p style="font-size:10pt;">${initialsLine(t)}</p>
  ...
  ${t.specialProvisions && t.specialProvisions.trim() ? `
  <h2>14. Special Provisions</h2>
  <p><strong>Notwithstanding any other provision of this Agreement</strong>, the parties expressly agree to the following. These Special Provisions control over and supersede any conflicting term elsewhere in this Agreement, including without limitation Section 4 (Permitted Use).</p>
  <ol style="margin:6px 0 0 20px;padding:0;">
    ${t.specialProvisions.split(/\r?\n/).filter((l) => l.trim()).map((l) => `<li style="margin:5px 0;">${esc(l.trim())}</li>`).join('')}
  </ol>
  <p style="font-size:10pt;margin-top:10px;">${initialsLine(t, { provider: true })}</p>
  ` : ''}

  <div class="sigs">
    <div><span class="sig-h">TENANT</span>
      ${t.signature?.dataUrl
        ? `<div class="sig-row">Signature: <img src="${t.signature.dataUrl}" alt="Tenant signature" style="height:50px;max-width:230px;vertical-align:middle;"></div>
           <div class="sig-row">Printed Name: <span class="sig-line">${esc(t.signature.name || c.name || '')}</span></div>
           <div class="sig-row">Date: <span class="sig-line">${prettyDate(t.signedAt)}</span></div>`
    ...
    <div><span class="sig-h">PROVIDER</span>
      ${t.providerSignature?.dataUrl
        ? `<div class="sig-row">Signature: <img src="${t.providerSignature.dataUrl}" alt="Provider signature" style="height:50px;max-width:230px;vertical-align:middle;"></div>
           <div class="sig-row">Printed Name: <span class="sig-line">${esc(t.providerSignature.name || 'Anton Bajada Leonardes')}</span></div>
           <div class="sig-row">Title: <span class="sig-line">Owner / Managing Member</span></div>
           <div class="sig-row">Date: <span class="sig-line">${prettyDate(t.executedAt)}</span></div>`
        : `<div class="sig-row">Signature: <span class="sig-line"></span></div>
  ...
  ${t.signature?.signedAt
    ? `<div style="margin-top:14px;border:1px solid #ccd5e0;border-radius:8px;padding:10px 14px;font-size:8.5pt;color:#556;">
         <strong style="color:${'#12356b'};">Electronic signature record.</strong>
         Signed electronically by ${esc(t.signature.name || '')} on
         ${new Date(t.signature.signedAt).toLocaleString('en-US')}${t.signature.ip ? ` from IP ${esc(t.signature.ip)}` : ''}.
         The signer confirmed intent for this electronic signature to be the legal equivalent of a
         handwritten signature (E-SIGN Act / Florida UETA).
       </div>`
    : ''}

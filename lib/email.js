...
// ---- Combined monthly Whiskey statement to Marc ----
async function renderWhiskeyStatementBody(spots, monthName) {
  const { whiskeySplit } = await import('./format.js');
  let total = 0;
  const rows = spots.map((s) => {
    const sp = whiskeySplit(s);
    total += sp.owed;
    return `<tr>
      <td style="padding:8px 6px;border-bottom:1px solid #e6eaf0;">${esc(s.name)}</td>
      <td style="padding:8px 6px;border-bottom:1px solid #e6eaf0;">${esc(s.spot || '-')}</td>
      <td style="padding:8px 6px;border-bottom:1px solid #e6eaf0;text-align:right;">${money(sp.rent)}</td>
      <td style="padding:8px 6px;border-bottom:1px solid #e6eaf0;text-align:right;">${sp.tontonPct}%</td>
      <td style="padding:8px 6px;border-bottom:1px solid #e6eaf0;text-align:right;font-weight:700;">${money(sp.owed)}</td>
    </tr>`;
  }).join('');
  total = Math.round(total * 100) / 100;

  const inner = `
    <h2 style="color:${NAVY};margin-top:0;">Monthly Whiskey Spot Statement</h2>
    <p style="color:#556;margin-top:0;">${esc(monthName)}</p>
    <p>Hi Marc, below is your monthly Whiskey Spot summary.</p>
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;margin:14px 0;">
      <thead><tr style="background:${NAVY};color:#fff;">
        <th style="padding:8px 6px;text-align:left;font-size:11px;">CUSTOMER</th>
        <th style="padding:8px 6px;text-align:left;font-size:11px;">SPOT</th>
        <th style="padding:8px 6px;text-align:right;font-size:11px;">MONTHLY RENT</th>
        <th style="padding:8px 6px;text-align:right;font-size:11px;">TONTON SHARE</th>
        <th style="padding:8px 6px;text-align:right;font-size:11px;">AMOUNT DUE</th>
      </tr></thead>
      <tbody>${rows || '<tr><td colspan="5" style="padding:12px 6px;color:#889;">No active spots.</td></tr>'}</tbody>
    </table>
    <div style="text-align:right;font-size:16px;font-weight:800;color:${NAVY};padding:8px 6px;border-top:2px solid ${NAVY};">
      Total amount due to TonTon: ${money(total)}
    </div>
    <p style="margin-top:20px;">Please let me know once payment has been sent.</p>
    <p style="margin:0;">Thank you,<br>TonTon Trailer Rentals LLC</p>`;

  return { html: emailShell(inner, `Whiskey Spot statement - ${money(total)} due`), total };
}

// Preview (no send) - returns the full email HTML + total.
export async function whiskeyStatementPreview(spots, monthName) {
  return renderWhiskeyStatementBody(spots, monthName);
}

export async function sendWhiskeyStatement(spots, monthName, toEmail) {
  const { html, total } = await renderWhiskeyStatementBody(spots, monthName);
  await sendEmail({
    to: toEmail,
    cc: [providerEmail()],
    subject: `TonTon Trailer Rentals - Monthly Whiskey Spot Statement - ${monthName}`,
    html,
  });
  return { total };
}
